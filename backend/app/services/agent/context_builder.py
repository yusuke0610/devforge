"""Agent チャット用の参照コンテキスト構築（GitHub 分析サマリー）。

DB アクセスはこのモジュールに閉じ込める。chat_service は DB に触れない。
GitHub コンテキストは career_summary / self_pr スコープのみに付与し、
project / experience では None を返す（スコープゲート）。

参照データの取得失敗（未連携・processing 中含む）は
...
def build_reference_context(db: Session, user_id: str, scope: str) -> dict | None:
    """career_summary / self_pr スコープ向けに GitHub 参照コンテキストを構築する。
