...
//   見ており、実際に攻撃面へ到達可能かは区別しない。到達不能と判断できる advisory を
//   個別に時限許容するために allowlist を持つ。
//   **原則は「脆弱パッケージを更新して解消する」**。allowlist は前進修正版が存在しない等で
//   更新できない場合の一時退避であり、修正版が出たら更新してエントリを削除する。
//
// 注意:
