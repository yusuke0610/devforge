...
| [ADR-0026](./0026-resume-draft-project-scope.md) | 経歴書ドラフトのプロジェクト明細化と候補選択制への縮小 | LLM / Agent | GitHub に無い職歴（会社・役割・工程）のプレースホルダ生成を廃止し、出力を project 明細へ縮小。リポジトリの採否を自動選定から人間の候補選択へ移し、注入は置換でなく追加にする |
...
| [ADR-0018](./0018-github-resume-draft-generation.md) | GitHub 連携データからの経歴書ドラフト生成 | Accepted | LLM / Agent | 関連: 0010（不変条件を継承し適用範囲を拡張）、0012（課金配線）、0013・0015（プロバイダ）、0016（データ供給源）、0020（非同期化・最小永続化で更新）、0026（出力単位を project へ / キャッシュ保存内容を拡張） | P4・P5 |
...
| [ADR-0020](./0020-async-resume-draft-generation.md) | 経歴書ドラフト生成の非同期化と最小永続化 | Accepted | LLM / Agent | 関連: 0018（同期実装を更新）、0010（不変条件を継承）、0012（課金をタスク側へ移設）、0016（データ供給源）、0026（非同期構造は維持のまま入出力の形を変更） | P1・P4・P5 |
...
| [ADR-0025](./0025-resume-draft-form-injection.md) | 経歴書ドラフトのフォーム流し込み（payload の JSON 公開） | Accepted | LLM / Agent | 継承: 0010（DB 非更新）、0018・0020（生成設計は不変）。手本: 0024（注入機構 #524）。実装: #525。0026 が注入のマージ規則（配列の置換）を改訂 | P4・P5・P6 |
| [ADR-0026](./0026-resume-draft-project-scope.md) | 経歴書ドラフトのプロジェクト明細化と候補選択制への縮小 | Accepted | LLM / Agent | 改訂: 0018（出力単位・キャッシュ保存内容）、0025（注入のマージ規則）。継承: 0010（DB 非更新）、0016（機械=幅/人間=深さ）、0020（非同期構造は不変）。手本: 0022（縮小/撤去の判断軸） | P4・P5・P6・P1 |
