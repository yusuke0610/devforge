...
| 3 ✅完了 | フォーム入出力含む | `src/types.ts`（手書き DTO 全体）を削除し、resume / blog / master の全 DTO を `api/types.ts` 再エクスポート層 + BE 名へ完全移行（`CareerResumeResponse`→`ResumeResponse`、`CareerResumePayload`→`ResumeCreate`、`CareerExperience`→`Experience`、`CareerClient`→`Client`、`CareerProject`→`Project`、`CareerTechnologyStack`→`TechnologyStackItem`、`ResumeQualification`→`ResumeQualificationItem`、`BlogAccount`→`BlogAccountResponse`、`BlogArticle`→`BlogArticleResponse`）。論点A（Experience/Client/Project の -Input/-Output 分裂）は構造同一のため読み取り側 -Output を canonical 採用、送信は ResumeCreate。論点B（list/team の optional 化）に伴い `payloadBuilders.ts` 検証ループ・`formMappers.ts`・`ContributionHeatmap` 等を null 安全化。`*Form` クライアント型は対象外で温存。`make ci` green・E2E 22 passed・codegen 冪等を確認済み | 中〜高 |
