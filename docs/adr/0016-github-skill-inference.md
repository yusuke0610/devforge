...
- **(g) 確定の解除（リセット / バラす）を提供する（2026-07 追加 / #496）**: 一度確定した表示名・畳み込みを機械デフォルトへ戻す導線を設ける。`DELETE /skills/display-decisions`（body に identity 群）が当該 Layer 3 確定行を削除し、解決順は自動的に「機械 `display_name` > canonical」へ戻る。同一グループの全メンバー identity をまとめて渡せば畳み込みも解ける（バラす）。authz は confirm と同じ（identity は当該ユーザーの検出済みスキルに属すること／存在しない確定行の指定は冪等に無視）。upsert（削除しない）設計では確定行が残り続け完全リセットできなかったギャップ（#496）を埋める。web はスキル一覧の確定済みチップに「解除」ボタンを出す。

**スコープ外（残課題）**: 確定値のバージョニング / 監査履歴、group をまたぐ evidence の重み再計算、agent による足切り（言語の上位 N 位）提案（D8 で別項）。動的 module 解決・Tier2 IaC は D10 の残課題のまま。確定単位での**メンバー個別の分割・除外**（グループから一部だけ外す）は未実装で、現状は「グループ丸ごと解除 → 再提案」で対応する。
...
- **2026-07**: D11 に**確定の解除（リセット / 畳み込みをバラす）導線を追加（#496・D11 項 (g)）**。`DELETE /skills/display-decisions`（identity 群指定）で Layer 3 確定行を削除し機械デフォルトへ完全に戻す（グループ全メンバー指定で畳み込みも解除）。upsert（削除しない）設計で確定行が残り続け完全リセットできなかったギャップを解消。web はスキル一覧の確定済みチップに「解除」ボタンを出す。schema 追加（`SkillDisplayResetRequest`）で `make codegen-types` 再生成。3 層モデル・D1〜D10 は不変。
